...
class DCGANGenerator(Generator):
    ...
    with Deep Convolutional Generative Adversarial Networks, Radford et al., 2016;
    https://arxiv.org/abs/1511.06434
    """
    def __init__(
            self, 
        ...
        # [N, nz] -> [N, ngf*8*4*4] --reshape--> [N, ngf*8, 4, 4] 
        self.projection = nn.Linear(nz, ngf*8*4*4, bias=False)
        self.net = nn.Sequential(
            # [N, ngf*8, 4, 4] -> [N, ngf*8, 4, 4] 
            nn.BatchNorm2d(8 * ngf),
            nn.ReLU(True),
            
            # [N, ngf*8, 4, 4] -> [N, 4*ngf, 8, 8] 
